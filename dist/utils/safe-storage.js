 ...
 */
import { reportCorruption } from './report-corruption.js';
/**
 ...
 */
export function safeLoadFromStorage(key, validate, source, defaultValue) {
    let raw;
    try {
    ...
    reportCorruption(source, `localStorage "${key}" contained valid JSON but failed type validation; removing`);
    try {
        localStorage.removeItem(key);
    }
    catch (removeErr) {
 ...
 */
export function safeSaveToStorage(key, value, source) {
    let serialized;
    try {
 ...
 */
export function safeRemoveFromStorage(key, source) {
    try {
        localStorage.removeItem(key);
        return true;
    }
