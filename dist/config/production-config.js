...
import { safeLoadFromStorage, safeRemoveFromStorage, safeSaveToStorage } from '../utils/safe-storage.js';
import { bytesToMb } from '../lib/metrics-utils.js';
/**
 ...
 */
const isPositiveFiniteNumber = (v) => typeof v === 'number' && Number.isFinite(v) && v > 0;
/**
 ...
 */
const isBooleanValue = (v) => typeof v === 'boolean';
/**
...
const isAllowedEnumValue = (value, allowed) => typeof value === 'string' && allowed.includes(value);
/**
...
export class ProductionConfigManager {
    ...
    getEnvironmentConfig() {
        const env = ProductionConfigManager.getNodeEnv();
        const configs = {
            development: {
                name: 'development',
    ...
    static validateConfigOverrides(parsed) {
        ...
        if ('apiBaseUrl' in parsed && typeof parsed.apiBaseUrl !== 'string')
            return false;
        // Check name type + declared environment set if present
        if ('name' in parsed && !isAllowedEnumValue(parsed.name, PROD_CONFIG_ENUM_FIELDS.name))
            return false;
        ...
        if ('features' in parsed) {
            ...
            if ('realTimeProcessing' in feat && !isBooleanValue(feat.realTimeProcessing))
                return false;
            if ('advancedAnalytics' in feat && !isBooleanValue(feat.advancedAnalytics))
                return false;
            if ('multiLanguageSupport' in feat && !isBooleanValue(feat.multiLanguageSupport))
                return false;
            if ('batchProcessing' in feat && !isBooleanValue(feat.batchProcessing))
                return false;
            if ('collaborativeEditing' in feat && !isBooleanValue(feat.collaborativeEditing))
                return false;
            if ('enterpriseFeatures' in feat && !isBooleanValue(feat.enterpriseFeatures))
                return false;
            if ('experimentalFeatures' in feat && !isBooleanValue(feat.experimentalFeatures))
                return false;
        }
        ...
        if ('performance' in parsed) {
            ...
            if (p === null || typeof p !== 'object' || Array.isArray(p))
                return false;
            const perf = p;
            if ('maxConcurrentJobs' in perf && !isPositiveFiniteNumber(perf.maxConcurrentJobs))
                return false;
            if ('timeoutMs' in perf && !isPositiveFiniteNumber(perf.timeoutMs))
                return false;
            if ('maxFileSize' in perf && !isPositiveFiniteNumber(perf.maxFileSize))
                return false;
            if ('memoryLimit' in perf && !isPositiveFiniteNumber(perf.memoryLimit))
                return false;
            if ('cacheStrategy' in perf && !isAllowedEnumValue(perf.cacheStrategy, PROD_CONFIG_ENUM_FIELDS.cacheStrategy))
                return false;
            if ('optimizationLevel' in perf && !isAllowedEnumValue(perf.optimizationLevel, PROD_CONFIG_ENUM_FIELDS.optimizationLevel))
                return false;
            if ('enableCompression' in perf && !isBooleanValue(perf.enableCompression))
                return false;
        }
        ...
        if ('monitoring' in parsed) {
            ...
            if (m === null || typeof m !== 'object' || Array.isArray(m))
                return false;
            const mon = m;
            if ('enableErrorTracking' in mon && !isBooleanValue(mon.enableErrorTracking))
                return false;
            if ('enablePerformanceMonitoring' in mon && !isBooleanValue(mon.enablePerformanceMonitoring))
                return false;
            if ('enableUserAnalytics' in mon && !isBooleanValue(mon.enableUserAnalytics))
                return false;
            if ('metricsCollectionInterval' in mon && !isPositiveFiniteNumber(mon.metricsCollectionInterval))
                return false;
            if ('logLevel' in mon && !isAllowedEnumValue(mon.logLevel, PROD_CONFIG_ENUM_FIELDS.logLevel))
                return false;
            ...
            if ('alertThresholds' in mon) {
                ...
                if (at === null || typeof at !== 'object' || Array.isArray(at))
                    return false;
                const thresholds = at;
                if ('errorRate' in thresholds && !isPositiveFiniteNumber(thresholds.errorRate))
                    return false;
                if ('responseTime' in thresholds && !isPositiveFiniteNumber(thresholds.responseTime))
                    return false;
                if ('memoryUsage' in thresholds && !isPositiveFiniteNumber(thresholds.memoryUsage))
                    return false;
            }
        }
        ...
        if ('export' in parsed) {
            ...
            if (e === null || typeof e !== 'object' || Array.isArray(e))
                return false;
            const exp = e;
            if ('concurrentExports' in exp && !isPositiveFiniteNumber(exp.concurrentExports))
                return false;
            if ('defaultFormat' in exp && !isAllowedEnumValue(exp.defaultFormat, PROD_CONFIG_ENUM_FIELDS.defaultFormat))
                return false;
            if ('compressionEnabled' in exp && !isBooleanValue(exp.compressionEnabled))
                return false;
            if ('watermarkEnabled' in exp && !isBooleanValue(exp.watermarkEnabled))
                return false;
            ...
            if ('qualityPresets' in exp) {
                ...
                for (const preset of qp) {
                    if (preset === null || typeof preset !== 'object' || Array.isArray(preset))
                        return false;
                    const p = preset;
                    if ('width' in p && !isPositiveFiniteNumber(p.width))
                        return false;
                    if ('height' in p && !isPositiveFiniteNumber(p.height))
                        return false;
                    if ('fps' in p && !isPositiveFiniteNumber(p.fps))
                        return false;
                    if ('quality' in p && !isPositiveFiniteNumber(p.quality))
                        return false;
                }
            }
        }
     ...
     */
    loadConfigOverrides() {
        const parsed = safeLoadFromStorage('production-config-overrides', (v) => v !== null && typeof v === 'object' && !Array.isArray(v) &&
            ProductionConfigManager.validateConfigOverrides(v), 'ProductionConfig', {});
