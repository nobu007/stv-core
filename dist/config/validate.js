...
export function validateConfig(config) {
    ...
    if (config.complexityThreshold !== undefined) {
        const rangeError = validateNumberRange(config.complexityThreshold, 0, 1, 'complexityThreshold');
        if (rangeError) {
            errors.push(rangeError);
        }
    }
    if (config.similarityThreshold !== undefined) {
        const rangeError = validateNumberRange(config.similarityThreshold, 0, 1, 'similarityThreshold');
        if (rangeError) {
            errors.push(rangeError);
        }
    }
